...
$query = "SELECT xml_data, num_base_pts, activity_score, weight_score, warmth_score, hardness_score FROM vase WHERE vase_id = '$left_vase_id';";
	if ($result = mysqli_query($_viv_sess_db, $query)) {
		
    		while ($row = mysqli_fetch_row($result)) {
	        	$left_vase_xml = $row[0];
			$left_base_pts = $row[1];
			$activity_score = round($row[2]*100);
			$weight_score = round($row[3]*100);
			$warmth_score = round($row[4]*100);
			$hardness_score = round($row[5]*100);
	    	}
	} else {
...
<div id="main" class="row">
    <div class="row">
        <a href="upload.html" class="button" id="previousbutton">&lt;&lt;</a>
    </div>
    <div class="row">
...
<div class="row">
    <div class="small-2 medium-2 large-2 columns">
        <div class="tweaklabel">
            <p>Weight: <?php echo $weight_score ?>%</p>
            <p>Temperature: <?php echo $warmth_score ?>%</p>
            <p>Activity: <?php echo $activity_score ?>%</p>
            <p>Hardness: <?php echo $hardness_score ?>%</p>
        </div>
    </div>
